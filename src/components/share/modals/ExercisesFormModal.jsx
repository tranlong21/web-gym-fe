...
const ExercisesFormModal = ({ visible, onClose, onSubmit, initialData }) => {
  ...
  if (initialData) {
    const parsedTechnique = (() => {
      const raw = initialData.technique_description;
      if (Array.isArray(raw)) return raw;
      if (typeof raw === "string") {
        // loại bỏ dấu chấm cuối, tách theo "."
        return raw
          .trim()
          .replace(/\.$/, "")
          .split(".")
  ...
  const handleSubmit = () => {
    ...
    const dataToSubmit = {
      ...
      rest_between_sets: Number(formData.rest_between_sets),
      muscle_group_id: Number(formData.muscle_group_id),
      technique_description: formData.technique_description.join(". ") + ".",
    };

    console.log("📤 Gửi từ Modal:", dataToSubmit);
